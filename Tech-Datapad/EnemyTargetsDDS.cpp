...
private:
    ...
    void drawHeaderText(MCUFRIEND_kbv &tftlcd, int16_t centerX, int16_t centerY, int16_t radius)
    {
        ...
        _ddsGraphicalUtility.printCenteredText(tftlcd, ENEMY_TARGET_HEADER_B_TEXT, &Aurebesh4pt7b, centerX, textTop, &textWidth, &textHeight, DISPLAY_RING_COLOR);
    ...
    void drawInnerCircles(MCUFRIEND_kbv &tftlcd, int16_t centerX, int16_t radius, int16_t topY)
    {
        ...
        _ddsGraphicalUtility.drawInnerCircle(tftlcd, centerX, innerCircleCenterY, innerCircleRadius, GRID_OUTER_FRAME_OFFSET, DISPLAY_RING_COLOR);
